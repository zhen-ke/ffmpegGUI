...
const Dropdown: React.FC<DropdownProps> = ({
    ...
    (event: React.KeyboardEvent) => {
      if (!isOpen) {
        if (event.key === 'Enter' || event.key === ' ' || event.key === 'ArrowDown') {
          ...
          setIsOpen(true);
          if (options.length > 0) {
            setHighlightedIndex(value ? options.findIndex((o) => o.id === value.id) : 0);
          }
        }
        return;
      }
  ...
  const TriggerIcon = value
    ? value.isCustom
      ? <Sparkles size={15} className="flex-shrink-0 text-purple-500 dark:text-purple-400" />
      : <FileCode size={15} className="flex-shrink-0 text-primary-500 dark:text-primary-400" />
    : <FileCode size={15} className="flex-shrink-0 text-slate-400 dark:text-slate-500" />;
